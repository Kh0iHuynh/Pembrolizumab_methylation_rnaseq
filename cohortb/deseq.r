...
write.table(res, file = "deseq_result_cohorta_survival.txt", col.names = TRUE, quote = FALSE, row.names = TRUE,sep = "\t")
...
write.table(normalized_counts, file = "normalized_counts_cohorta.txt", sep = "\t", quote = FALSE, col.names = TRUE, row.names = TRUE)
...
write.table(vp, file = "percent_variance_explained_cohorta.txt", sep = "\t", quote = FALSE, row.names = TRUE)

# 6. Summary Output
cat("DESeq2 analysis completed.\n")
cat("Variance Partitioning results saved to 'percent_variance_explained.txt'.\n")

data <- read.table("percent_variance_explained_cohorta.txt",header =  TRUE, row.names = 1,sep ="\t")
data2 <- read.table("deseq_result_cohorta_survival.txt",header =  TRUE, row.names = 1,sep ="\t")

data2 = data2 %>% mutate(gene=rownames(data2))
data = data %>% mutate(gene=rownames(data))

data3 <- inner_join(data, data2, by = c("gene" = "gene"))
write.table(data3, file = "joint_deseq_result_percent_variance_explained_cohorta.txt", sep = "\t", quote = FALSE, row.names = FALSE)
