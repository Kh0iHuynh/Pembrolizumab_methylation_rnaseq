...
mann_whitney_results <- alldata2 %>%
  filter(Cohort == "A") %>%  # Filter for Cohort B
...
write.table(mann_whitney_results, file = "mann_whitney_methylation_cohorta_result.txt", append = FALSE, quote = FALSE, row.names = FALSE)
